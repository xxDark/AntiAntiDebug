...
public final class AntiAntiDebugPlugin implements StartupPlugin {
  ...
  private static final String UNSAFE =
      VMUtil.getVmVersion() > 8 ? "jdk.internal.misc.Unsafe" : "sun.misc.Unsafe";
  private static final String UNSAFE_INTERNAL = internalName(UNSAFE);
  ...
  private static final String CLASS_LOADER = "me.xdark.antiantidebug.WrappedClassLoader";
  private static final String LOADER = "me.xdark.antiantidebug.LoaderInjector";
  private static final String LOADER_INTERNAL = internalName(LOADER);
  ...
  private static byte[] toCode(ClassNode node) {
    return ClassUtil.toCode(node, ClassWriter.COMPUTE_FRAMES);
  }
  ...
  private static byte[] renameClass(ClassNode target, String oldClass, String newClass) {
    ClassWriter writer = new ClassWriter(0);
    Remapper remapper = new Remapper() {
      @Override
      public String mapType(String internalName) {
        return oldClass.equals(internalName) ? newClass : internalName;
      }
    };
    target.accept(new ClassRemapper(writer, remapper));
  ...
  private static void injectBootstrapClasses()
      throws Exception {
    Class<?> klass = Class.forName(UNSAFE, true, null);
    ...
    loadBootstrapClass(CLASS_LOADER, unsafe, define);
    loadBootstrapClass(LOADER, unsafe, define);
  }
  ...
  private static void registerClassLoadHook(Instrumentation instrumentation) {
          ...
          public MethodVisitor visitMethod(int access, String name, String descriptor,
            ...
            return new MethodVisitor(Opcodes.ASM8,
              ...
              public void visitMethodInsn(int opcode, String owner, String name, String descriptor,
                  boolean isInterface) {
                // :)
                if (UNSAFE_INTERNAL.equals(owner)) {
                  if ("defineClass".equals(name)) {
                    // unsafe [args]
                    super.visitMethodInsn(Opcodes.INVOKESTATIC, LOADER_INTERNAL, name, descriptor,
                        isInterface);
                    // unsafe class
  ...
  public void onStart(Controller controller) {
    ...
    if (instrumentation != null) {
      ...
      registerClassLoadHook(instrumentation);
